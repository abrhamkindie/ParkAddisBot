...
export function registerFavorites(bot) {
  ...
  bot.callbackQuery(/^favorites:list$/, async (ctx) => {
  ...
  bot.callbackQuery(/^favorite:add:(\d+)$/, async (ctx) => {
  ...
  bot.callbackQuery(/^favorite:remove:(\d+)$/, async (ctx) => {
    const spotId = Number(ctx.match[1]);
    await ctx.answerCallbackQuery();
  ...
  bot.callbackQuery(/^favorite:rebook:(\d+)$/, async (ctx) => {
    const spotId = Number(ctx.match[1]);
    await ctx.answerCallbackQuery();
